...
**New to Python or command lines?** See the [complete step-by-step setup guide](SETUP.md) — no prior experience assumed, ~20–30 minutes for a complete beginner.
...
You'll also need your own [Anthropic API key](https://console.anthropic.com) — this is separate from your Spectandi key, and it's what lets `tracked_chat()` actually call Claude on your behalf.

### Configuration

All three of these can be set as environment variables (e.g. in a `.env` file, loaded automatically):

| Variable            | Required | Description                                                                            |
| ------------------- | -------- | -------------------------------------------------------------------------------------- |
| `SPECTANDI_API_KEY` | Yes      | Authenticates trace uploads to Spectandi.                                              |
| `ANTHROPIC_API_KEY` | Yes      | Authenticates the underlying call to Claude. `tracked_chat()` calls the Anthropic API on your behalf. |
...
This is an early release (0.x) — the interface may evolve as we learn from real usage. Feedback welcome at [hello@spectandi.com](mailto:hello@spectandi.com).
